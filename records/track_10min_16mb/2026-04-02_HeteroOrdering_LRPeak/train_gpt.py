...
try:
    ...
    def flash_attn_3_func(q, k, v, causal=True):
        # FA3 layout: (B, T, H, D) → SDPA needs (B, H, T, D)
...
class Hyperparameters:
    ...
    # --- Data ordering & training dynamics (our additions) ---
    data_order = os.environ.get("DATA_ORDER", "none")  # none/homo/hetero/hard_hetero
    lr_warmup_frac = float(os.environ.get("LR_WARMUP_FRAC", "0.0"))
    lr_peak = float(os.environ.get("LR_PEAK", "1.0"))
    # Trim easiest sequences per shard (by BoW PCA score variance — low variance = boilerplate)
    # 0.0 = keep all, 0.1 = drop easiest 10%. Only active when data_order != none.
    trim_easy = float(os.environ.get("TRIM_EASY", "0.0"))
...
class TokenStream:
    def __init__(self, pattern: str, order: str = "none", seq_len: int = 1024,
        ...
        self.trim_easy = trim_easy
        self._pc_lookup: np.ndarray | None = None
        self.file_idx = 0
        self.tokens = load_data_shard(self.files[0])
        if self.order != "none":
            self.tokens = self._reorder(self.tokens)
        self.pos = 0
    ...
    def _fit_bow_pca(self, seqs: np.ndarray) -> None:
        ...
        self._pc_lookup = Vt[0].astype(np.float32)

    def _reorder(self, tokens: Tensor) -> Tensor:
        sl = self.seq_len
        ...
        # Trim easiest sequences (lowest absolute PCA score = least distinctive)
        if self.trim_easy > 0:
            abs_scores = np.abs(scores)
            threshold = np.percentile(abs_scores, self.trim_easy * 100)
            keep_mask = abs_scores[sorted_idx] >= threshold
        ...
        elif self.order == "hard_hetero":
            # Interleave diversity, but biased toward the hard (high-score) end:
            # split into quartiles by score, interleave within each, concat hard-first
            q1 = n_full // 4
            chunks = [sorted_idx[3*q1:], sorted_idx[2*q1:3*q1], sorted_idx[q1:2*q1], sorted_idx[:q1]]
            parts = []
            for c in chunks:
                nc = len(c)
                ne, no = (nc+1)//2, nc//2
                o = np.empty(nc, dtype=np.int64)
                o[0::2] = c[:ne]
                o[1::2] = c[nc-1:nc-1-no:-1]
                parts.append(o)
...
class DistributedTokenLoader:
    def __init__(self, pattern: str, rank: int, world_size: int, device: torch.device,
                 order: str = "none", seq_len: int = 1024, trim_easy: float = 0.0):
        self.rank = rank
        self.world_size = world_size
        self.device = device
        self.stream = TokenStream(pattern, order=order, seq_len=seq_len, trim_easy=trim_easy)
...
def main() -> None:
    ...
    def lr_mul(step: int, elapsed_ms: float) -> float:
        ...
        if args.lr_peak > 1.0 and args.lr_warmup_frac > 0:
            ...
            if t < warmdown_t and warmdown_t > args.lr_warmup_frac:
                decay_t = (t - args.lr_warmup_frac) / (warmdown_t - args.lr_warmup_frac)
                return args.lr_peak + decay_t * (1.0 - args.lr_peak)
        # Warmdown (original logic)
    ...
    def est_total(step: int, elapsed_ms: float) -> str:
        if max_wallclock_ms is None or step < 1 or elapsed_ms <= 0:
            return str(args.iterations)
        return f"~{min(args.iterations, int(max_wallclock_ms / (elapsed_ms / step)))}"
    ...
    while True:
        ...
        if should_validate:
            ...
            log0(
                f"step:{step}/{est_total(step, training_time_ms)} val_loss:{val_loss:.4f} val_bpb:{val_bpb:.4f} "
        ...
        if last_step:
            if stop_after_step is not None and step < args.iterations:
                log0(
                    ...
                    f"step:{step}/{est_total(step, training_time_ms)}"
                )
            break
        elapsed_ms = training_time_ms + 1000.0 * (time.perf_counter() - t0)
        scale = lr_mul(step, elapsed_ms)
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear._qat_enabled:
            # Only enable QAT during warmdown, not during warmup (where scale also starts low)
            progress = _training_progress(step, elapsed_ms) if step > 0 else 0.0
            if progress > 0.5:
        ...
        if should_log_train:
            log0(
                f"step:{step}/{est_total(step, training_time_ms)} train_loss:{train_loss.item():.4f} "
